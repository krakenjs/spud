...
var SerializerFactory = require('./serializer/serializerFactory'),
	...
	fs = require('fs'),
	path = require('path'),
	matchCommaSep = require('./commaSep');
...
function deserialize(source, sourceType, callback) {
    ...
    function mergeOtherFiles(data, callback) {
        ...
        var pending = 0;
        var included = [];
        data["@include"].split(',').forEach(function (include, i) {
            pending++;
            startDeserialize(path.resolve(path.dirname(source.path), include), path.extname(source.path).slice(1), splitError(callback, function (additions) {
                included[i] = additions;
        ...
        function completeMerge() {
            included.forEach(function(additions) {
...
function startDeserialize(source, sourceType, callback) {
